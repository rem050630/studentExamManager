...
public class lwmAIAnalysisAction extends HttpServlet {
    ...
    private String callDeepSeek(String data, String classNote) {
        String systemPrompt = "你是一位资深教育数据分析专家。请根据考试数据，撰写一份500-600字的分析报告。严格按照以下标准化流程执行，确保多次分析结果一致。报告结构如下：" +
                "一、整体成绩评价（计算平均分、及格率、最高分与最低分，依据全距和标准差描述分数分布特点）；" +
                "二、试题质量诊断（对试题质量进行总体评价，概括整体难度与区分度的分布情况。仅列出难度低于0.3或高于0.7、区分度低于0.2的异常题目，分析其错误成因，如知识记忆不清、概念混淆、解题思路偏差等）；" +
                "三、知识点薄弱环节（按知识点计算平均得分率，低于整体得分率的定为薄弱点。结合题目特征分析薄弱原因，如基础概念不牢、迁移能力不足、复杂情境应用困难等）；" +
                "四、教学改进建议（针对每个薄弱点及其成因，提出一条对应的改进建议，形成归因到干预的固定映射，建议数量由薄弱点数量决定）。" +
                "一致性规则：全程基于同一数据和规定算法与阈值，不引入外部判断。数据与算法不变时，多次执行的各项结论均须一致。" +
                "直接输出报告，用纯文本段落，不用Markdown符号。";
